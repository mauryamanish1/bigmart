...
selected_features = ['Item_Weight', 'Item_Fat_Content', 'Item_Visibility', 'Item_Type', 'Item_MRP', 'outlet_rel_age', 'Outlet_Type','visibility_outlet']
...
# Fill missing Item_Weight values
test_data1['Item_Weight'] = test_data1.groupby('Item_Identifier')['Item_Weight'].transform(lambda x: x.fillna(x.mean()))
test_data1['Item_Weight'].fillna(test_data1['Item_Weight'].mean(), inplace=True)
...
for col in test_data1.select_dtypes(include=['object']).columns:
    test_data1[col] = encoder.fit_transform(test_data1[col])
